...
st.caption("Load `data.csv` — the Ireland & England coastal water quality dataset.")
...
if df is not None:
    ...
    k4.metric("Date From",        str(df["date"].min().date()))
    k5.metric("Date To",          str(df["date"].max().date()))
