...
from core.classifier import benchmark_models, classify, generate_recommendations
...
def predict(
    ...
    # Train model bundle if none provided.
    model_bundle = classifier_bundle
    if model_bundle is None:
        try:
            model_bundle = benchmark_models(df)
        except Exception:
            model_bundle = None
    ...
    for metric in METRICS:
        ...
        # Build historical + forecast series for chart
        hist = d[["date", metric]].dropna().set_index("date")[metric].rename("Historical")
